...
const LoginForm = ({ isAdmin = false, onSuccess, onSwitchMode }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className={`text-center mb-6 ${isAdmin ? 'bg-red-50 p-4 rounded-lg border border-red-200' : ''}`}>
        <h2 className={`text-2xl font-bold mb-2 ${isAdmin ? 'text-red-800' : 'text-gray-900'}`}>
          {isAdmin ? 'Admin Login' : 'Welcome back'}
        </h2>
        <p className={`${isAdmin ? 'text-red-600' : 'text-gray-600'}`}>
          {isAdmin ? 'Sign in to your admin account' : 'Sign in to your Stylon account'}
        </p>
        {isAdmin && (
